  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (currentTask < bootTasks.length) {
        ...
        setProgress(100)
        setTimeout(onComplete, 800) // Chờ 1 chút sau khi 100% rồi mới tắt
      }
    }, 1000) // Tốc độ chạy từng dòng log (Anh có thể chỉnh số này lớn hơn để câu giờ lâu hơn)
...
const MagneticButton = ({ children, className = "", onClick }: any) => {
  ...
  const move = (e: React.MouseEvent) => { const { clientX, clientY } = e; const { left, top, width, height } = ref.current!.getBoundingClientRect(); setPos({ x: (clientX - (left + width/2)) * 0.3, y: (clientY - (top + height/2)) * 0.3 }) }
  return (
...
const FloatingElement = ({ children, duration = 3, yOffset = 10 }: any) => (
  <motion.div animate={{ y: [0, -yOffset, 0] }} transition={{ duration, repeat: Infinity, ease: "easeInOut" }}>{children}</motion.div>
)
  ...
  return (
    <>
      ...
      <motion.div 
        ref={targetRef} 
        initial={{ opacity: 0 }}
        animate={{ opacity: isSystemBooted ? 1 : 0 }} // Chỉ hiện khi đã boot xong
        ...
        <AnimatePresence>
          {isDemoOpen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[9999] bg-black/95 backdrop-blur-3xl flex flex-col items-center justify-center p-4">
              ...
              <button onClick={() => setIsDemoOpen(false)} className="absolute top-8 right-8 group z-50 flex items-center gap-3 px-5 py-3 rounded-full border border-white/10 bg-black hover:border-[#00ff88]/50 transition-all">
              ...
              <div className="flex justify-center">
                <div onClick={() => setIsDemoOpen(true)} className="group relative cursor-pointer">
                    ...
                    <div className="relative w-64 h-64 border border-white/10 bg-black/50 backdrop-blur-md rounded-full flex items-center justify-center group-hover:border-[#00ff88]/50 transition-colors duration-500">
                      ...
                      <div className="text-center"><Cpu size={48} className="mx-auto mb-4 text-[#00ff88] group-hover:scale-125 transition-transform duration-300" /><div className="text-xs font-bold tracking-widest text-white group-hover:text-[#00ff88]">ACCESS CORE</div></div>
                    </div>
                </div>
              </div>
          </div>
